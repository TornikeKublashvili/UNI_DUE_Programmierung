...
public class PhoneBook {
	...
	private void insertPerson(Person newPerson, Person currentNode){
		if(newPerson == null) {
			...
			currentNode = newPerson;
		}
		else {
			Person links = newPerson.getLeftSuccessor();
			Person rechts = newPerson.getRightSuccessor();
			newPerson.setLeftSuccessor(null);
			newPerson.setRightSuccessor(null);
			...
			insertPerson(links, currentNode);
			insertPerson(rechts, currentNode);
		}
	...
	private boolean findPerson(String lastName, String firstName, int number, Person currentNode){
		if(currentNode != null) {
			if(currentNode.getLastName().equals(lastName) && currentNode.getFirstName().equals(firstName) 
				...
				return true;
			}else if(findPerson(lastName, firstName, number, currentNode.getLeftSuccessor()) == true) {
				return true;
			}else if(findPerson(lastName, firstName, number, currentNode.getRightSuccessor()) == true) {
				return true;
			}
		}
		return false;
	...
	public Person[] findAllPersonsByFirstName(String firstName){
		return findAllPersonsByFirstName(firstName, root, new Person[0]);
	}
	
	// Rueckgabetyp und Parametrisierung dieser Methode duerfen geaendert werden!
	private Person[] findAllPersonsByFirstName(String firstName, Person currentNode, Person[]parr){
		if(currentNode == null) {
			return parr;
		}
		parr = findAllPersonsByFirstName(firstName, currentNode.getLeftSuccessor(), parr);
		
		if(currentNode.getFirstName().equals(firstName)) {
			parr = add(currentNode, parr);
		}
		
		parr = findAllPersonsByFirstName(firstName, currentNode.getRightSuccessor(), parr);

		return parr;
	}
	public static Person [] add(Person x, Person[]y) {
		if(x!= null) {
			if(y == null) {
				Person neu[] = new Person [1];
				neu[0] = x; 
				
				return neu;
			}
			else {
				Person neu[] = new Person[y.length +1];
	...
	public void removePersonFromPhoneBook(String lastName, String firstName, int number){
		 root = removePersonFromPhoneBook(lastName, firstName, number, root);

	}
	
	// Rueckgabetyp und Parametrisierung dieser Methode duerfen geaendert werden!
	private Person removePersonFromPhoneBook(String lastName, String firstName, int number, Person currentNode){
		   Person p = new Person(lastName, firstName, number, false);
		   if(currentNode == null) {
			   return null;
		   }else {
			   if(compareTwoPersons(p, currentNode) == 0) {
				   if(currentNode.getRightSuccessor()== null) {
					   return currentNode.getLeftSuccessor();
				   }else {
					   insertPerson(currentNode.getLeftSuccessor(), currentNode.getRightSuccessor());
					   return currentNode.getRightSuccessor();
				   }
			   }
			   currentNode.setLeftSuccessor(removePersonFromPhoneBook(lastName, firstName, number, currentNode.getLeftSuccessor()));
			   currentNode.setRightSuccessor(removePersonFromPhoneBook(lastName, firstName, number, currentNode.getRightSuccessor()));
		   }
	...
	private void changePerson(String lastName, String firstName, int number, String newLastname, Person currentNode){
		Person p = new Person (newLastname, firstName, number, true);
		if(findPerson(lastName, firstName, number, currentNode)) {
	...
	private static void testFindPersons() {
		Person person1 = new Person("Meier", "Rainer", 1, false);
		Person person2 = new Person("Meier", "Rainer", 2, false);
		Person person3 = new Person("Neumann", "Rainer", 3, false);
		Person person4 = new Person("Mustermann", "Max", 4, false);
		Person person5 = new Person("Otto", "Reiner", 5, false);
		Person person6 = new Person("Fischer", "Oskar", 6, false);
